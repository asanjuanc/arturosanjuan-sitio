...
subtitle: "Apuntes de clase: cancelación catastrófica, pérdida de significancia y número de condición"
...
Calculemos $\kappa_f(x)$ para $f(x)=\dfrac{e^x-1}{x}$ (extendida por continuidad a $f(0)=1$).

$$f'(x)=\frac{xe^x-(e^x-1)}{x^2}=\frac{(x-1)e^x+1}{x^2}$$

$$\kappa_f(x)=\left|x\cdot\frac{f'(x)}{f(x)}\right| = \left|\frac{(x-1)e^x+1}{e^x-1}\right|$$

Expandiendo en serie de Taylor alrededor de $x=0$:

$$(x-1)e^x+1 = \frac{x^2}{2}+\frac{x^3}{3}+\cdots \qquad e^x-1=x+\frac{x^2}{2}+\cdots$$

$$\kappa_f(x) \approx \frac{x^2/2}{x} = \frac{|x|}{2} \;\xrightarrow[x\to 0]{}\; 0$$

Numéricamente: $\kappa_f(10^{-6})\approx 5\times10^{-7}$, $\kappa_f(1)\approx 0{,}58$, $\kappa_f(10)\approx 9$, $\kappa_f(20)\approx 19$ — crece linealmente solo para $x$ grande, nunca "explota". El **problema** $f(x)=(e^x-1)/x$ es entonces muy bien condicionado cerca de $x=0$: $\kappa_f(0)=0$.

Y sin embargo, si evalúas $f$ en una máquina con esta fórmula tal cual —calcular $e^x$, restarle 1, dividir por $x$— para $x$ pequeño el resultado es basura. Esto no es una contradicción: es la distinción central del capítulo entre **condicionamiento del problema** y **estabilidad del algoritmo**.

### Qué pasa exactamente en la resta $e^x-1$

Cuando $x$ es pequeño, $e^x$ está muy cerca de $1$ — por ejemplo $e^{10^{-8}} = 1{,}00000000999999994\ldots$ En doble precisión ($\approx16$ cifras significativas), ese número se guarda con toda su precisión disponible, pero al restarle $1$ ocurre exactamente la cancelación de la primera sección: los primeros 8 dígitos (el "$1{,}0000000$" que comparten $e^x$ y $1$) se cancelan exactamente, y lo único que sobrevive son los dígitos que ya estaban cerca del límite de precisión de la máquina. El resultado hereda el ruido de redondeo de $e^x$ como si fuera señal.

Comparemos, para $x=10^{-8}$, la fórmula naíf y la función segura `expm1` (diseñada para evitar la resta):

| cantidad | valor calculado |
|---|---|
| $e^x$ (doble precisión) | $1{,}00000000999999994$ |
| $e^x-1$ (naíf) | $9{,}99999993922529\times10^{-9}$ |
| `expm1(x)` (seguro) | $1{,}00000000500000\times10^{-8}$ |
| valor de referencia | $1{,}00000000500000\times10^{-8}$ |

El resultado naíf tiene error relativo $\approx 1{,}1\times10^{-8}$ — apenas **8 dígitos correctos**, cuando la máquina en principio te puede dar 16. `expm1` da el valor exacto hasta el redondeo de máquina. Con $x=10^{-12}$ la cosa empeora: la fórmula naíf se queda con solo **4 dígitos correctos**, mientras `expm1` sigue exacta.
